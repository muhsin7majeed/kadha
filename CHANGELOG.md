...
- Increased search-result density on wide screens and made narrow layouts adapt card columns without overflow or unnecessary single-card whitespace.
...
- Removed the unused `ts-node` dependency and retired the unreferenced legacy server Dockerfile.
...
- Added a prominent, plain-language explanation of why end-to-end encryption is deferred during beta and what hosted operator access means in the meantime.
