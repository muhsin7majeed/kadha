      ...
      {data.map(({ media, contextualDate }) => {
        ...
        return (
          <Grid
            ...
            gridTemplateColumns={
              showActions
                ? { base: '64px minmax(0, 1fr) auto', md: '88px minmax(0, 1fr) auto' }
                : { base: '64px minmax(0, 1fr)', md: '88px minmax(0, 1fr)' }
            }
            gap={{ base: '3', md: '4' }}
            alignItems="start"
            borderWidth="1px"
          ...
          >
            ...
            <Stack gap="2" minW="0">
              ...
              <Flex gap={{ base: '2', md: '4' }} wrap="wrap" color="fg.muted" textStyle="compactLabel">
                <Text>{getMediaRuntime(media)}</Text>
                <Text>TMDB {getMediaScore(media)}</Text>
                {showPersonalRating && <Text>Your rating {media.rating != null ? `${media.rating / 2}/5` : '—'}</Text>}
                {contextualDate && <Text>{contextualDate.label} {contextualDate.value}</Text>}
              </Flex>
            </Stack>

            {showActions && (
              <Box alignSelf="start">
                <MediaActions media={media} orientation={{ base: 'column', md: 'row' }} size="xs" />
              </Box>
            )}
          </Grid>
        );
