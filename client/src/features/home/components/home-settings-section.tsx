...
import {
  ...
  HOME_SECTION_DETAILS,
} from "@/features/home/home-defaults";
import type {
  HomePreferenceItem,
  HomePreferences,
...
const HomeSettingsSection = () => {
  ...
  const isDefault =
    JSON.stringify(preferences) === JSON.stringify(DEFAULT_HOME_PREFERENCES);

  const updateItems = (
    updater: (items: HomePreferenceItem[]) => HomePreferenceItem[],
  ) => {
      ...
      <Card.Body pt="0">
        <Stack gap="5">
          <ReorderablePreferenceList
            ...
            renderControls={(item) => (
              <SimpleCheckbox
                ariaLabel={`Show ${HOME_SECTION_DETAILS[item.id].label} on Home`}
          ...
          <HStack gap="3" flexWrap="wrap">
            ...
            <Button
              ...
              onClick={() =>
                setPreferences(structuredClone(DEFAULT_HOME_PREFERENCES))
              }
            >
              <LuRotateCcw />
              Restore defaults
