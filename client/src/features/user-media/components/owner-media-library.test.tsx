...
const baseProps = {
  ...
  emptyState: {
    ...
    description: "Like something first.",
    icon: <LuHeart />,
  },
  error: null,
...
describe("owner media library", () => {
  ...
  it("switches display modes without changing the library query", () => {
    ...
    expect(
      getComputedStyle(gridButton).getPropertyValue(
        "--chakra-colors-color-palette-solid",
      ),
    ).toBe("var(--chakra-colors-brand-solid)");
    expect(
      getComputedStyle(listButton).getPropertyValue(
        "--chakra-colors-color-palette-solid",
      ),
  ...
  it("renders retained results in each selected display mode", () => {
    ...
    expect(
      screen.getByRole("table", { name: "Liked library table" }),
    ).toBeInTheDocument();
    expect(screen.queryByText(media.overview)).not.toBeInTheDocument();
  });
  ...
  it("uses a theme-aware Chakra checkbox for genre filters", async () => {
    renderWithProviders(
      <OwnerMediaLibrary {...baseProps} updateQuery={vi.fn()} />,
    );

    fireEvent.click(screen.getByRole("button", { name: "Filters" }));
    const genreCheckbox = await screen.findByRole("checkbox", {
      name: "Drama",
    });
    const checkboxRoot = genreCheckbox.closest(
      '[data-scope="checkbox"][data-part="root"]',
    );

    expect(checkboxRoot).not.toBeNull();
    expect(
      checkboxRoot?.querySelector('[data-part="control"]'),
    ).toBeInTheDocument();
  });
  ...
  it("debounces title search and commits type and sort controls", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Search this library"), {
    ...
    expect(
      screen.getByRole("option", { name: "Recently liked" }),
    ).toBeInTheDocument();
  });
  ...
  it("uses the media-type pills without a redundant chip and can hide rating controls", async () => {
    ...
    expect(
      screen.queryByRole("option", { name: "Highest personal rating" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByText("10 titles match your filters"),
    ).toBeInTheDocument();
  ...
  it("batches match-all genres, years, and rating behind Apply", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Filters" }));
    const dramaCheckbox = await screen.findByRole("checkbox", {
      name: "Drama",
    });
    ...
    fireEvent.change(screen.getByLabelText("Through year"), {
      target: { value: "2020" },
    });
  ...
  it("keeps controls visible for filtered-empty results and clears all criteria in one action", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "No titles match these filters" }),
    ).toBeInTheDocument();
  ...
  it("qualifies placeholder results and keeps search available while locking conflicting controls", () => {
    ...
    expect(
      screen.queryByText("100 titles match your filters"),
    ).not.toBeInTheDocument();
  ...
  it("repairs an out-of-range restored page without showing a false empty state", async () => {
    ...
    await waitFor(() =>
      expect(updateQuery).toHaveBeenCalledWith({ page: 2 }, { replace: true }),
    );
    expect(
  ...
  it("keeps a truly empty base library distinct even with restored criteria", () => {
    ...
    expect(
      screen.queryByRole("heading", { name: "No titles match these filters" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("shows runtime sorting only for a single media type and contextual ascending copy", () => {
    ...
    expect(
      screen.queryByRole("option", { name: "Shortest runtime" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: "First liked" }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByRole("option", { name: "Shortest runtime" }),
    ).toBeInTheDocument();
  });
  ...
  it("blocks invalid year ranges and removes applied chips individually", async () => {
    ...
    const yearError = screen.getByText(
      /Use whole years from 1970 through 2026/,
    );

    expect(fromYearInput).toHaveAttribute("aria-describedby", yearError.id);
    expect(screen.getByLabelText("Through year")).toHaveAttribute(
      "aria-describedby",
      yearError.id,
    );
    expect(
